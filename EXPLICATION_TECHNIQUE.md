...
**Les LEDs Neopixel (WS2812B)**  
Ce sont 24 LEDs RGB enchaînées en série, une par emplacement.  
Elles s'allument en différentes couleurs selon l'état de chaque slot :
- **Bleu** → batterie chargée, disponible à la location
- **Blanc dim** → batterie en charge (moins de 80%)
- **Rouge** → batterie défectueuse ou anomalie, non disponible
- **Vert** → emplacement déverrouillé, la batterie vient d'être retirée
- **Éteint** → emplacement vide, en maintenance

Toutes les LEDs sont pilotées par **une seule broche GPIO (GPIO 18)**, le signal se propage de LED en LED.
...
**La détection de présence de batterie**  
Chaque emplacement a des connecteurs pogo-pins (petites broches à ressort).  
Quand une batterie est insérée, elle complète un circuit → un optocoupleur génère un signal **HIGH** sur la broche d'entrée correspondante.  
Le code lit toutes ces broches **toutes les 500 ms** pour savoir si une batterie est présente ou non.
...
1. Cherche dans la base locale si cette carte a une **session active** (= batterie déjà louée)
   - Si **oui** → c'est un **retour** : on vérifie que la batterie est bien dans l'emplacement, on ferme la session, la LED passe au blanc
   - Si **non** → c'est une **demande de location** :
     - On contacte le backend cloud : `POST /api/rent`
     - Si le backend répond : il donne le numéro du slot à ouvrir → le solénoïde s'ouvre, la LED passe au vert
...
- **HIGH → LOW** (batterie retirée) :
  - Si une session est active → retrait autorisé (l'étudiant est parti avec)
  - Sinon → **retrait non autorisé** : on log une alerte, la LED passe au rouge, on marque l'emplacement comme défectueux

#### Thread 3 — Boucle de surveillance de la charge
Toutes les 60 secondes, lit les niveaux de charge stockés en base (mis à jour par la synchronisation cloud) et ajuste les couleurs des LEDs :
- Charge ≥ 80% → **bleu** (prêt à louer)
- Charge < 80% → **blanc** (en charge)
- Défectueux → **rouge**

#### Thread 4 — Boucle de synchronisation cloud
Toutes les 30 secondes :
1. Envoie les événements en file d'attente vers le backend (accumulés pendant une coupure réseau)
2. Pousse les sessions créées hors ligne
3. Récupère les états à jour des slots (niveaux de charge, défectueux)
...
Contient : slot_id (1-24), battery_uid (quel batterie est dedans), is_locked, led_state, charge_level, is_defective  
→ Mise à jour à chaque événement physique et à chaque sync cloud
...
Chaque insertion, retrait, déverrouillage, anomalie est enregistré avec un timestamp
...
battery_uid (RFID UID de la batterie elle-même), charge_level, cycle_count, health_status (GOOD/DEGRADED/DEFECTIVE)
...
Lié à une station et à une batterie, is_locked, led_state, is_defective
...
**`SlotLog`** — audit trail avec champ JSONB pour les détails
...
**Synchronisation :**
- `POST /api/sync/push` → la borne envoie ses événements accumulés hors ligne. Le backend répond avec la liste des cartes autorisées et les états à jour des slots
...
- `PUT /api/admin/slots/<id>/flag-defective` → marquer un emplacement comme défectueux
...
5. Si connecté → **StationScreen** : l'app appelle `GET /api/slots/<station_id>` et affiche une grille de 24 cases colorées
6. L'étudiant **tape sur un slot bleu** (disponible) → **RentConfirmScreen**
...
9. L'étudiant retire la batterie
...
Affiche chaque slot avec sa couleur et son niveau de charge  
Slot bleu = tappable  
...
Chaque case (`SlotCard`) connaît son `led_state` et affiche la couleur correspondante.  
Si `led_state = "BLUE"`, la case est cliquable. Sinon, elle affiche une alerte.
...
Étudiant tape sa carte
    ...
    → LED passe au vert (GPIO 18 → WS2812B)
...
```
Étudiant re-tape sa carte
    → Thread RFID lit l'UID
    → Trouve une session active pour cet UID en SQLite
    → Thread de surveillance confirme: batterie présente dans le slot ? (détection HIGH)
    → OUI → Session fermée (end_time = maintenant)
    → LED passe au blanc (en charge)
...
1. **4 threads en parallèle sur le Pi** : RFID, détection physique, surveillance charge, synchronisation cloud — chacun indépendant, chacun résistant aux erreurs
...
3. **Détection physique fiable** : la présence d'une batterie est détectée par un circuit électrique (pogo pins → optocoupleur → signal HIGH), pas par logiciel seul

4. **Sécurité à plusieurs niveaux** : clé API pour les bornes, JWT pour les étudiants, tokens chiffrés sur le téléphone, aucune donnée sensible dans le code

5. **Adaptabilité universitaire** : import CSV ou push JSON pour importer les étudiants, variables d'environnement pour tout ce qui est institution-spécifique

6. **LED comme interface utilisateur** : les 24 LEDs sont la principale interface visuelle de la borne — elles donnent un retour immédiat sans écran
